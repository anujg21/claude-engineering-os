...
| Date | What | Evidence |
| --- | --- | --- |
| 2026-09-08 | Added a definition-of-ready check to `/plan`: five self-check gates (no placeholders, references resolve, check runnable now, done-when binary, dependencies named) a drafted task clears before it goes in the plan table. Lightweight analogue of `ai-sdlc-framework/ai-sdlc`'s DoR gate, without its agent/scoring/admission machinery. | `skills/plan/SKILL.md`, `docs/engineering/lifecycle.md` Phase 6 |
